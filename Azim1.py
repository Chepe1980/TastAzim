...
class EnhancedSyntheticGenerator:
    ...
    def _generate_stress_field(self, nx: int, ny: int, nz: int,
        ...
        sigma_v = -0.022 * z_coords[:, np.newaxis, np.newaxis]  # MPa/m gradient
        
        # Horizontal stresses
        # Typically: sigma_h < sigma_v < sigma_H
        ...
        # Stress concentration near faults
        for fault_pos in [nx//4, nx//2, 3*nx//4]:
            for i in range(nx):
                distance = abs(i - fault_pos)
                if distance < 10:
                    stress_concentration = np.exp(-distance/5)
                    sigma_H[:, :, i] *= (1 + 0.2 * stress_concentration)
        ...
        return {
            'vertical': sigma_v,
            'horizontal_min': sigma_h,
            'horizontal_max': sigma_H,
            'orientation': stress_orientation,
            'pore_pressure': sigma_v * 0.4  # Hydrostatic
        }
            ...
            for j in range(ny):
                for k in range(nx):
                    ...
                    fluid_props = {
                        ...
                        'fluid_K': self.model.fluid_properties[frac['fill']]['K'],
                        'fluid_density': self.model.fluid_properties[frac['fill']]['rho'],
    ...
    def _generate_well_logs(self, elastic_props: Dict, geology: Dict,
        ...
        ny, nx = geology['Vp'].shape[1], geology['Vp'].shape[2]
        ...
        for well in well_locations:
            ...
            logs = {
                'depth': np.arange(geology['Vp'].shape[0]) * config['dz'],
                'Vp': geology['Vp'][:, y, x],
                'Vs': geology['Vs'][:, y, x],
                'rho': geology['rho'][:, y, x],
...
class EnhancedVisualization:
    ...
    def plot_comprehensive_results(results: Dict):
        ...
        # 13-16. 3D plots using plotly (conceptual - would embed in Streamlit)
...
def create_streamlit_app():
    ...
    if app_mode == "Model Configuration":
        ...
        with col1:
            ...
            if st.button("Calculate AVAz Response"):
        ...
        with col2:
            if st.button("Run Fluid Substitution"):
        ...
        if st.button("Run Inversion (Demo)"):
