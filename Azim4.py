...
class EnhancedSyntheticGenerator:
    ...
    def _generate_geology_from_well_logs(self, nx: int, ny: int, nz: int,
        ...
        # Create 3D grids
        Vp = np.zeros((nz, ny, nx))
        Vs = np.zeros((nz, ny, nx))
        rho = np.zeros((nz, ny, nx))
        porosity = np.zeros((nz, ny, nx))
        clay_content = np.zeros((nz, ny, nx))
        ...
        for well in well_positions:
            if well['name'] in self.well_logs:
                logs = self.well_logs[well['name']]
                well_data.append({
                    'x': well['x'],
                    'y': well['y'],
                    'depth': logs['DEPTH'].values if 'DEPTH' in logs.columns else np.arange(len(logs)),
                    'Vp': logs['VP'].values if 'VP' in logs.columns else None,
                    'Vs': logs['VS'].values if 'VS' in logs.columns else None,
                    'rho': logs['RHOB'].values if 'RHOB' in logs.columns else None,
                    'phi': logs['PHIE'].values if 'PHIE' in logs.columns else None,
                    'sw': logs['SW'].values if 'SW' in logs.columns else None,
                    'gr': logs['GR'].values if 'GR' in logs.columns else None,
                    'rt': logs['RT'].values if 'RT' in logs.columns else None
                })
        ...
        for depth_idx in range(nz):
            depth_value = depth_idx * config['dz']
            
            # Collect values at this depth for each well
            vp_values = []
            vs_values = []
            rho_values = []
            phi_values = []
            ...
            for well in well_data:
                ...
                depth_diff = np.abs(well['depth'] - depth_value)
                if len(depth_diff) > 0:
                    closest_idx = np.argmin(depth_diff)
                    
                    if well['Vp'] is not None and closest_idx < len(well['Vp']):
                        vp_values.append(well['Vp'][closest_idx])
                        well_coords.append([well['x'], well['y']])
                    
                    if well['Vs'] is not None and closest_idx < len(well['Vs']):
                        vs_values.append(well['Vs'][closest_idx])
                    
                    if well['rho'] is not None and closest_idx < len(well['rho']):
                        rho_values.append(well['rho'][closest_idx])
                    
                    if well['phi'] is not None and closest_idx < len(well['phi']):
                        phi_values.append(well['phi'][closest_idx])
                    
                    if well['gr'] is not None and closest_idx < len(well['gr']):
                        gr_values.append(well['gr'][closest_idx])
            
            if len(vp_values) >= 2:  # Need at least 2 points for interpolation
                well_coords = np.array(well_coords)
                
                # Interpolate Vp
                if len(vp_values) >= 2:
                    interp_vp = interpolate.griddata(well_coords, vp_values, (X, Y), 
                                                     method='linear', fill_value=np.mean(vp_values))
                    Vp[depth_idx] = interp_vp.T
                
                # Interpolate Vs
                if len(vs_values) >= 2:
                    interp_vs = interpolate.griddata(well_coords, vs_values, (X, Y), 
                                                     method='linear', fill_value=np.mean(vs_values))
                    Vs[depth_idx] = interp_vs.T
                else:
                    # Estimate Vs from Vp using Castagna's relation
                    Vs[depth_idx] = Vp[depth_idx] / 1.8
                
                # Interpolate density
                if len(rho_values) >= 2:
                    interp_rho = interpolate.griddata(well_coords, rho_values, (X, Y), 
                                                      method='linear', fill_value=np.mean(rho_values))
                    rho[depth_idx] = interp_rho.T
                else:
                    # Estimate density from Gardner's relation
                    rho[depth_idx] = 310 * (Vp[depth_idx] ** 0.25) * 0.23  # Modified Gardner
                
                # Interpolate porosity
                if len(phi_values) >= 2:
                    interp_phi = interpolate.griddata(well_coords, phi_values, (X, Y), 
                                                      method='linear', fill_value=np.mean(phi_values))
                    porosity[depth_idx] = interp_phi.T
                else:
                    # Default porosity
                    porosity[depth_idx] = 0.2
                
                # Estimate clay content from GR
                if len(gr_values) >= 2:
                    interp_gr = interpolate.griddata(well_coords, gr_values, (X, Y), 
                                                     method='linear', fill_value=np.mean(gr_values))
                    # Convert GR to clay content (simplified)
                    gr_min = np.min(gr_values)
                    gr_max = np.max(gr_values)
                    if gr_max > gr_min:
                        clay_content[depth_idx] = (interp_gr.T - gr_min) / (gr_max - gr_min)
                    else:
                        clay_content[depth_idx] = 0.3
                else:
                    clay_content[depth_idx] = 0.3
            else:
                # Not enough data, use default values
                Vp[depth_idx] = 2500 + depth_idx * 1.5
                Vs[depth_idx] = Vp[depth_idx] / 1.8
                rho[depth_idx] = 2000 + 0.3 * Vp[depth_idx]
                porosity[depth_idx] = 0.25 - 0.0001 * depth_idx
                clay_content[depth_idx] = 0.3
    ...
    def _process_actual_well_logs(self, config: Dict) -> Dict:
        ...
        for well_name, df in self.well_logs.items():
            ...
            logs = {
                ...
                'Vp': df['VP'].values if 'VP' in df.columns else None,
                'Vs': df['VS'].values if 'VS' in df.columns else None,
                'rho': df['RHOB'].values if 'RHOB' in df.columns else None,
                'porosity': df['PHIE'].values if 'PHIE' in df.columns else None,
                'sw': df['SW'].values if 'SW' in df.columns else None,
                'gr': df['GR'].values if 'GR' in df.columns else None,
                'rt': df['RT'].values if 'RT' in df.columns else None
            }
            ...
            if logs['Vp'] is not None and logs['Vs'] is not None:
                ...
                vp_vs = logs['Vp'] / logs['Vs']
                vp_vs_norm = (vp_vs - np.nanmean(vp_vs)) / np.nanstd(vp_vs)
        ...
        with col2:
            if st.session_state.well_logs:
                ...
                if selected_well:
                    ...
                    log_columns = {
                        'Vp': ['VP', 'Vp', 'vp', 'P_VEL', 'P_VELOCITY'],
                        'Vs': ['VS', 'Vs', 'vs', 'S_VEL', 'S_VELOCITY'],
